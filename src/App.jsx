...
const mockData = {
    ...
    entities: {
        customers: [
            ...
            { id: 2, name: 'Global Traders', address: '456 Commerce St, New York', gst: '07HIJKL5678M2N9' },
            { id: 3, name: 'Sunrise Solutions (Salem)', address: '789 Industrial Estate, Salem, Tamil Nadu', gst: '33PQRST9101U3V4' },
        ],
        suppliers: [
...
const allPermanentTransactions = [
    ...mockData.transactions.sales,
    ...mockData.transactions.purchases,
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <div className={`w-full max-w-md m-auto flex flex-col rounded-xl shadow-2xl ${getBackgroundColor()} ${getTextColor()}`}>
                <div className="p-6 text-center">
                    ...
                    <h3 className="mt-4 text-lg font-semibold">Caution</h3>
...
const PeriodSelector = ({ theme }) => {
    ...
    const inputRef = useRef(null);
    const formatMonthYear = (date) => date.toLocaleString('default', { month: 'short' }) + '-' + date.getFullYear().toString().slice(-2);
...
const ThemeElements = ({ theme, toggleTheme, boldnessLevel, toggleBold, fontFamilies, fontIndex, selectFont }) => {
    ...
    useEffect(() => {
        const handleClickOutside = (event) => { if (fontDropdownRef.current && !fontDropdownRef.current.contains(event.target)) setIsFontDropdownOpen(false); };
        document.addEventListener('mousedown', handleClickOutside);
        return () => document.removeEventListener('mousedown', handleClickOutside);
    }, []);
    return (
        <div className="flex items-center space-x-2">
            <div className="relative" ref={fontDropdownRef}>
                <button onClick={() => setIsFontDropdownOpen(!isFontDropdownOpen)} className={`flex items-center p-2 h-10 rounded-md transition-colors duration-200 focus:outline-none ${theme === 'light' ? 'bg-gray-200 text-gray-800' : 'bg-gray-700 text-gray-100'}`}>
                    <Type size={20} /><ChevronDown size={20} className="ml-1" />
                </button>
                {isFontDropdownOpen && (
                    <div className={`absolute right-0 mt-2 w-48 rounded-md shadow-lg z-10 py-1 ${theme === 'light' ? "bg-white border-gray-200" : "bg-gray-800 border-gray-700"}`} style={{ fontFamily: 'Inter' }}>
                        {fontFamilies.map((font, index) => (
                            <button key={font} onClick={() => selectFont(index)} className={`block w-full text-left px-4 py-2 text-sm transition-colors duration-150 ${theme === 'light' ? 'text-gray-700 hover:bg-gray-100' : 'text-gray-100 hover:bg-gray-700'}`} style={{ fontFamily: font }}>
                                {font}
                            </button>
                        ))}
                    </div>
            ...
            <button onClick={toggleBold} className={`relative p-2 h-10 rounded-md transition-colors duration-200 focus:outline-none ${theme === 'light' ? 'bg-gray-200 text-gray-800' : 'bg-gray-700 text-gray-100'}`}>
                <Bold size={20} className={boldnessLevel > 0 ? (theme === 'light' ? "text-blue-600" : "text-blue-400") : ""} strokeWidth={2.5 + boldnessLevel * 0.5} />
            </button>
            <button onClick={toggleTheme} className={`relative p-2 h-10 rounded-md transition-colors duration-200 focus:outline-none ${theme === 'light' ? 'bg-gray-200 text-gray-800' : 'bg-gray-700 text-gray-100'}`}>
                {theme === 'light' ? <Sun size={20} className="text-yellow-500" strokeWidth={3} /> : <Moon size={20} className={theme === 'black' ? 'text-gray-400' : 'text-blue-500'} style={{ transform: 'rotate(270deg)' }} />}
            </button>
        </div>
    );
};
